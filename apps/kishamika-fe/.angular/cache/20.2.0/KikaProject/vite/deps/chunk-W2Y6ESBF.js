import { canUseDom, isTemplateRef, updateCSS, warn, wrapIntoObservable } from './chunk-U7XIMOED.js';
import {
  BarsOutline,
  CalendarOutline,
  CaretDownFill,
...
import { IconDirective, IconService } from './chunk-NHMQZRFA.js';
...
import { animationFrameScheduler, asapScheduler } from './chunk-HUZPAJME.js';
import { Subject, debounceTime, filter, finalize, map } from './chunk-PVVX4OOT.js';
...
var NzConfigService = class _NzConfigService {
  ...
  constructor() {
    if (this.config.theme) {
      registerTheme(
        this.getConfig().prefixCls?.prefixCls || defaultPrefixCls,
        this.config.theme,
        this.cspNonce,
      );
    }
  ...
  set(componentName, value) {
    this.config[componentName] = __spreadValues(
      __spreadValues({}, this.config[componentName]),
      value,
    );
  ...
  return function (_value, context) {
    context.addInitializer(function () {
      ...
      Object.defineProperty(this, context.name, {
        get: () => {
          const configValue = nzConfigService.getConfigForComponent(this['_nzModuleName'])?.[
            context.name
          ];
          if (assignedByUser) {
            return value;
  ...
  fetchFromIconfont(opt) {
    ...
    if (this._document && !this.iconfontCache.has(scriptUrl)) {
      ...
      this._renderer.setAttribute(
        script,
        'data-namespace',
        scriptUrl.replace(/^(https?|http):/g, ''),
      );
...
var NzStringTemplateOutletDirective = class _NzStringTemplateOutletDirective {
  ...
  updateContext() {
    const newCtx = isTemplateRef(this.nzStringTemplateOutlet)
      ? this.nzStringTemplateOutletContext
      : this.context;

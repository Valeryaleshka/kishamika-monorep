import { toSignal } from './chunk-FBBY4FQW.js';
import {
  Inject,
  Injectable,
  NgModule,
...
import { queueScheduler } from './chunk-HUZPAJME.js';
import {
  BehaviorSubject,
  Observable,
  Subject,
...
function createAction(type, config) {
  ...
  if (typeof config === 'function') {
    return defineType(type, (...args) =>
      __spreadProps(__spreadValues({}, config(...args)), {
        type,
      }),
    );
  }
...
function createActionGroup(config) {
  ...
  return Object.keys(events).reduce(
    (actionGroup, eventName) =>
      __spreadProps(__spreadValues({}, actionGroup), {
...
var _ACTION_TYPE_UNIQUENESS_CHECK = new InjectionToken(
...
function createFeatureReducerFactory(metaReducers) {
  const reducerFactory =
    Array.isArray(metaReducers) && metaReducers.length > 0 ? compose(...metaReducers) : (r) => r;
...
function extractArgsFromSelectorsDictionary(selectorsDictionary) {
  ...
  const projector = (...selectorResults) =>
    resultKeys.reduce(
      (result, key, index) =>
...
function createNestedSelectors(featureSelector, reducer) {
  ...
  return nestedKeys.reduce(
    (nestedSelectors, nestedKey) =>
      __spreadProps(__spreadValues({}, nestedSelectors), {
      ...
      useValue: {
        ...
        reducerFactory:
          !(config instanceof InjectionToken) && config.reducerFactory
            ...
            : combineReducers,
        metaReducers:
          !(config instanceof InjectionToken) && config.metaReducers ? config.metaReducers : [],
        initialState:
          !(config instanceof InjectionToken) && config.initialState ? config.initialState : void 0,
      },
    },
    {
...
//# sourceMappingURL=chunk-HRDWNVIR.js.map
